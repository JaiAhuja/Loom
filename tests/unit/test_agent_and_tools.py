...
from src.services.tools.rag_tool import _format_doc_result, _format_search, create_rag_tool
from src.services.tools.safe_graph_tool import _parse_params, _format_result, create_safe_graph_tool
...
def test_system_prompt_and_agent_routing_reflect_enabled_tools():
    ...
    assert should_continue({"messages": [AIMessage(content="", tool_calls=[{"name": "x", "args": {}, "id": "1", "type": "tool_call"}])]}) == "tools"


def test_document_tool_formatters_include_source_and_empty_state():
    doc = Document(
        page_content="retrieved text",
        metadata={"paper": "Paper A", "domain": "AI", "paper_chunk": "summary", "chunk_type": "summary"},
    )
