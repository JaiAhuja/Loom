...
class DocumentProcessor:
    ...
    Usage:
        ...
        result = processor.process("path/to/document.pdf", model="llama3.2")
        chunks = result["chunks"]
        markdown = result["markdown"]  # reuse for KG extraction
        profile = result["profile"]    # full LLM-extracted paper profile
    """
    ...
    # ----- Paper Name from Filename (fallback) -----
    ...
    def process(self, file_path: str, model: str = None, extra_metadata: dict = None,
        ...
        # Step 1 — Convert PDF → DoclingDocument (expensive — do this ONCE)
        ...
        # Persist the markdown text alongside the PDFs under data/txt/
        try:
        ...
        # Step 2 — Single LLM call for the full paper profile
        ...
        # Step 3 — Resolve display title + domain (LLM wins, filename falls back)
        ...
        # Step 4 — Hybrid chunking (operates on the DoclingDocument directly)
        ...
        # Step 6 — Summary chunk (prepended so it is always indexed)
        if summary_text:
        ...
        # Step 7 — Merge caller-supplied metadata (e.g. document_id, ingest_id)
        if extra_metadata:
            for doc in enriched:
                doc.metadata.update(extra_metadata)

        # Step 8 — Deterministic chunk_id for idempotent vector writes
        for doc in enriched:
